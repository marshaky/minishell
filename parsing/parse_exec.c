...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/28 15:00:42 by marshaky          #+#    #+#             */
/*   Updated: 2025/12/28 15:15:31 by mnaghdal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_input(char *cmd)
{
	...
	if (cmd && cmd[0] == '~' && ft_strlen(cmd) == 1)
	{
		temp = replace_word("~: Is a directory\n",
				"~", getenv("HOME"), 0);
		ft_putstr_fd(temp, 2);
		free(temp);
		return (126);
...
void	print_error(char *cmd, char *path, int *err)
{
	...
	{
		*err = 127;
		tmp = replace_word("E: command not found\n", "E", cmd, 0);
	}
	if (!*err && !tmp && !stat(path, &stats))
	{
		...
		{
			*err = 126;
			tmp = replace_word("E: Is a directory\n", "E", cmd, 0);
		}
		else if (access(path, X_OK))
		{
			*err = 126;
			tmp = replace_word("E: Permission denied\n", "E", cmd, 0);
		}
	}
	ft_putstr_fd(tmp, 2);
	free(tmp);
